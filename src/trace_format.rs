...
//! The Python bootstraps carry a byte-compatible encoder (`src/bootstrap_*.py`),
//! kept in lockstep with the constants and layout here.
//!
//! ## Layout
//!
//! ```text
...
// into a Slice); `slice` is the pre-closed interval stored in a `.glr`; `gc`
// appears in both.
pub const TY_SWITCH: u16 = 1;
pub const TY_GC: u16 = 2;
pub const TY_SLICE: u16 = 3;
...
impl Decoder {
    ...
    fn build_item(&self, type_id: u16, ts: u64, mut vals: Vec<Val>) -> Option<Item> {
        match type_id {
            ...
            // slice (ts = start): dur, gid, name, func, task, stack
            TY_SLICE if vals.len() >= 6 => {
                let dur = vals[0].as_u();
                let gid = vals[1].as_u();
...
impl Encoder {
    ...
    /// Emit the built-in `slice` + `gc` schemas (the set a `.glr` uses).
    ...
    pub fn slice(&mut self, s: &Slice) {
        ...
        let mut body = Vec::new();
        body.extend_from_slice(&TY_SLICE.to_le_bytes());
    ...
    /// bootstrap streams them. Mirrors `glr.switch` / the `slice` encoder above.
    ...
    /// Drop the buffered output bytes, keeping the interning/timestamp state. The
    /// file writer calls this after flushing `bytes()` to disk so a long recording
    /// doesn't hold the whole encoded stream in memory.
...
mod tests {
    ...
    #[test]
    fn slice_and_gc_roundtrip_through_glr_schemas() {
        ...
        // A second slice sharing func + stack → must dedupe to the same pool ids.
        let s2 = Slice {
            gid: 0x66,
            start: 2_000,
            dur: 10,
        ...
        assert_eq!(items.len(), 4); // meta, slice, slice, gc
...
"#;
    ...
    /// End-to-end: the real asyncio bootstrap in `all` mode streams a decodable
    /// stream of task switches, each carrying the loop's OS thread id, and (mode
    /// `all`, warn 0) a yield-point stack on the spans it closes.
    #[test]
    fn asyncio_bootstrap_streams_decodable_events() {
        let driver = r#"
...
async def main():
    await asyncio.gather(work(8), work(8), work(8))
...
try:
    ...
    pass
"#;
        let items = match run_bootstrap(crate::BOOTSTRAP_ASYNCIO, crate::TraceMode::All, 0, driver)
        {
            Some(i) => i,
            None => return, // skipped (no python3 / didn't connect)
        };
        assert!(
        ...
        assert!(
            sw.len() >= 2,
            "expected multiple task switches, got {}",
            sw.len()
        );
        assert!(
            sw.iter().all(|s| s.thread != 0),
            "switches carry the loop's OS thread id"
        );
        assert!(
            sw.iter().any(|s| s.label == "Loop"),
            "expected the synthetic Loop lane"
        );
        // `all` + warn 0 → at least one closed task step carries a yield stack.
        assert!(
            sw.iter().any(|s| !s.stack.is_empty()),
            "all-mode should capture some stacks"
        );
    }

    /// `slow` mode gates the walk: a long task step (over the warn threshold) gets a
    /// full stack; the many fast steps don't.
    #[test]
    fn asyncio_slow_mode_captures_only_slow_steps() {
        // ~30ms busy step in one task; everything else is sub-ms. Warn = 10ms.
        let driver = r#"
import asyncio, builtins, time

async def slow():
    t = time.perf_counter()
    while time.perf_counter() - t < 0.03:  # ~30ms on-CPU step (> warn)
        pass
    await asyncio.sleep(0)
...
async def main():
    await asyncio.gather(slow(), fast(20), fast(20))
...
try:
    ...
    pass
"#;
        let warn = 10_000_000; // 10ms
        let items = match run_bootstrap(
            crate::BOOTSTRAP_ASYNCIO,
            crate::TraceMode::Slow,
            warn,
            driver,
        ) {
            Some(i) => i,
        ...
        let sw = switches(&items);
        assert!(!sw.is_empty(), "expected switches");
        let with_stack = sw.iter().filter(|s| !s.stack.is_empty()).count();
        let without_stack = sw.iter().filter(|s| s.stack.is_empty()).count();
        assert!(
            with_stack >= 1,
            "the slow step should have a captured stack"
        );
        assert!(
            without_stack >= 1,
            "fast steps should NOT have walked stacks (slow-mode gating)"
        );
    }
...
try:
    ...
    pass
"#;
        let items = match run_bootstrap(crate::BOOTSTRAP_GEVENT, crate::TraceMode::All, 0, driver) {
            Some(i) => i,
            None => return,
        };
        assert!(
