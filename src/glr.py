...
def _wv(buf, v):
    # Append an unsigned LEB128 varint. Fast-path the common single-byte case
    # (every small id/delta on the hot path) to skip the loop setup.
    if v < 0x80:
        buf.append(v)
        return
    while True:
...
class GlrEnc:
    ...
    __slots__ = ("buf", "_si", "_ki", "_ns", "_nk", "base", "_scratch")

    # Upper bound on distinct interned strings / stacks. A target under sustained
    # high load with high-cardinality fields (e.g. a unique `request_id` per
    # request) would otherwise grow these pools — and the process's memory —
    # without limit. Past the cap, new strings degrade to id 0 ("") and new stacks
    # are dropped, rather than leak. Generous: real workloads have far fewer
    # distinct labels/funcs/stacks; this only trips on pathological cardinality.
    _MAX_POOL = 100_000
    ...
    def __init__(self, buf):
        ...
        self._nk = 1
        self.base = 0
        # Reused per-event scratch buffer (see `switch`): avoids allocating a fresh
        # bytearray on every switch. The framed copy into `buf` happens in `_frame`,
        # so it's safe to clear and refill in place each call.
    ...
    def str_id(self, s):
        ...
        if i is not None:
            return i
        if self._ns >= self._MAX_POOL:
            return 0  # pool full: degrade to "" rather than grow without bound
        i = self._ns
        self._ns = i + 1
        self._si[s] = i
        body = bytearray()
    ...
    def stack_id(self, frames):
        ...
        if i is not None:
            return i
        if self._nk >= self._MAX_POOL:
            return 0  # pool full: drop the stack rather than grow without bound
