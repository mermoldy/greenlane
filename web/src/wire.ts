...
export type GcEvent = { start: number; dur: number; gen: number; collected: number };
...
// Decode the server's binary `window` frame:
//   u32 LE headerLen | header JSON | pad→4 | f32 startMs[n] | f32 durMs[n] |
//   u32 trackIdx[n] | u32 funcIdx[n] | u32 taskIdx[n] | u32 stackIdx[n]
// Typed-array views point straight into the received buffer — no copy, no parse
// of the big per-slice data (only the small header is JSON). NOTE: `startMs` is
// **relative to the window's `t0`** (small values → f32 keeps sub-ms precision on
// long captures); the timeline adds back `t0 - origin` in f64 to get absolute ms.
export function decodeWindow(buf: ArrayBuffer) {
  ...
  // Validate the frame is long enough for all 6 columns before making views —
  // a truncated/corrupt frame must not throw an opaque out-of-bounds error.
  if (!(n >= 0) || off + n * 24 > buf.byteLength) {
  ...
  const funcIdx = col(Uint32Array) as Uint32Array;
  const taskIdx = col(Uint32Array) as Uint32Array;
  const stackIdx = col(Uint32Array) as Uint32Array;
  const tracks = header.tracks as WindowTrack[];
  const dict = header.dict as { func: string[]; task: string[]; stack: string[] };
  // Validate every index is in range BEFORE the caller applies the frame: an
  // out-of-range trackIdx/dict index would otherwise throw (or read undefined)
  // deep inside the renderer, outside this decode's try/catch. Cheap one pass
  // over the (window-capped) columns.
  checkIndices(trackIdx, tracks.length, "trackIdx");
  checkIndices(funcIdx, dict.func.length, "funcIdx");
  checkIndices(taskIdx, dict.task.length, "taskIdx");
  checkIndices(stackIdx, dict.stack.length, "stackIdx");
  return {
    req: (header.req as number) ?? 0,
    t0: header.t0 as number,
    t1: header.t1 as number,
    // Absolute ns bounds of the data actually returned (0/0 when empty).
