...
//   u32 LE headerLen | header JSON | pad→4 | f32 start | f32 dur | u32 ×4
function buildFrame(
  header: object,
  cols: {
    start: number[];
  ...
  const buf = new ArrayBuffer(off + n * 24);
  ...
  put(Uint32Array, cols.fi);
  put(Uint32Array, cols.ti);
  put(Uint32Array, cols.si);
  return buf;
}
...
const HEADER = {
  ...
  dict: { func: ["app.py:a:1"], task: [""], stack: [""] },
};

test("decodeWindow round-trips header fields and columns", () => {
  const buf = buildFrame(HEADER, {
    start: [0, 5],
    dur: [5, 1],
    trk: [0, 0],
    fi: [0, 0],
  ...
  expect(w.dict.func[0]).toBe("app.py:a:1");
...
test("decodeWindow reads the data bounds + retention horizon", () => {
  const buf = buildFrame(
    ...
    { start: [0, 5], dur: [5, 1], trk: [0, 0], fi: [0, 0], ti: [0, 0], si: [0, 0] },
  );
  const w = decodeWindow(buf);
...
test("decodeWindow rejects an out-of-range track index", () => {
  ...
  const buf = buildFrame(HEADER, {
    start: [0, 5], dur: [5, 1], trk: [0, 1], fi: [0, 0], ti: [0, 0], si: [0, 0],
  });
  expect(() => decodeWindow(buf)).toThrow(/trackIdx\[1\]=1 out of range/);
});

test("decodeWindow rejects an out-of-range dict index", () => {
  // dict.func has one entry (index 0 valid); claim funcIdx 3 for one row.
  const buf = buildFrame(HEADER, {
    start: [0, 5], dur: [5, 1], trk: [0, 0], fi: [0, 3], ti: [0, 0], si: [0, 0],
  });
  expect(() => decodeWindow(buf)).toThrow(/funcIdx\[1\]=3 out of range/);
});

test("decodeWindow handles an empty window (n=0)", () => {
  const buf = buildFrame(
    { ...HEADER, n: 0, counts: { visible: 0, total: 0 }, tracks: [], gc: [] },
    { start: [], dur: [], trk: [], fi: [], ti: [], si: [] },
  );
  const w = decodeWindow(buf);
...
test("decodeWindow rejects a truncated frame instead of reading OOB", () => {
  ...
  const buf = new ArrayBuffer(off + 24); // room for 1 row, not 8
  new DataView(buf).setUint32(0, hbytes.length, true);
  new Uint8Array(buf, 4, hbytes.length).set(hbytes);
